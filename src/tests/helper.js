...
const directiveLettersLength = 5

if (regexData.test(codigo)) {
  obj.data = true
}

if (regexWord.test(codigo)) {
  obj.word = codigo.substring(
    regexWord.exec(codigo).index + directiveLettersLength,
    regexText.exec(codigo).index
  )
  .trim()
  .split(' ')
}

if (regexText.test(codigo)) {
  obj.text = codigo.substring( regexText.exec(codigo).index + directiveLettersLength )
}
