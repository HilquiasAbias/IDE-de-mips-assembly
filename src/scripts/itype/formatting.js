...
export function formatInstructionsInBinary(arr) {
    const imm = formatImmElement(arr.pop());
    const func = instructions[arr.shift()].function;

    const cleanedElements = arr.map((element, index) => tools.convertDecimalToBin(parseInt(tools.cleanElement(element))));

    return [ func, ...tools.completeElementsLength(cleanedElements), imm ]; // [ '001000',  ]
}


// formatInstructionsInBinary